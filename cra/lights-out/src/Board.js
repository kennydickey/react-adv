  ...
  createBoard() {
    ...
    for(let y = 0; y < this.props.nrows; y++) {
      ...
      for(let x = 0; x < this.props.ncols; x++) {
        row.push(Math.random() < this.props.chanceLightStartsOn) //pushing in t or f
      }
      board.push(row);
    }
    return board
  ...
  render() {
    ...
    if(this.state.hasWon) {
      ...
      <div className='winner'>
        yay!
        <div className="second">you win!</div>
      </div>
      )
    }
    ...
    for(let y = 0; y < this.props.nrows; y++) {
      ...
      tblBoard.push(<tr key={y}>{row}</tr>) //out of llop as to not creat many boards
    }
    return (
      <div>
        <div className="title">
